...
parser.add_argument('--file_domains',help = 'Path to file with functional domain positions in format 1\n1\n , default = []', default=[] )
...
def Read_Domains(List_domains,File_domains):
    ...
    if not List_domains:
        if not File_domains:
            ...
            List_domains = [line.rstrip('\n') for line in open(File_domains)]
            List_domains = list(map(int, List_domains))
    else:
        # Convert the input list in string format to a string by 'executing' it
        List_domains = eval(List_domains)
    return List_domains

List_domains = Read_Domains(List_domains,File_domains)
