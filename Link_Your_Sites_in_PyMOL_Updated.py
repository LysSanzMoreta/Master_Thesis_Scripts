...
def Corresponding_Coordinates_and_labels_PDB_Gene(PDB,Gene,Full_PDB_sequence,List_of_Positive_Positions,functional_list,Residues_ID,basepath,print_alignment,Clean_positions = None):
    ...
    #List = equivalent_positions(PDB, Gene, Aligned_A, Aligned_B) #List of equivalent positions of each of the gene residues into the PDB
...
def Pymol():
    ...
    def Colour_by_Selection(selection="all",
        ...
        cmd.color('gray', 'all')
        #ROTATION
        cmd.rotate([0,1,0], angle = 180,selection = "all") # Commands to rotate the structures to visualize some specific side of the protein [x,y,z]
        ...
        cyl_text(cgo, plain, [70.0, 60.0, 80.0], '%s' % (sname.split('.')[0]), radius=0.6, color=[0.0, 0.0, 0.0],axes=axes)  # x=60 for RIOK2, x=40 and z=60 for ROS1
        cyl_text(cgo, plain, [70.0, 50.0, 80.0], 'Positively Selected', radius=0.6, color=[1.0, 0.5, 0.0], axes=axes)
        cyl_text(cgo, plain, [70.0, 40.0, 80.0], 'Not selected', radius=0.6, color=[0.5, 0.5, 0.5], axes=axes)
        cyl_text(cgo, plain, [70.0, 30.0, 80.0], 'Functional Domain', radius=0.6, color=[0.5, 1.0, 0.5], axes=axes)
        cyl_text(cgo, plain, [70.0, 20.0, 80.0], 'Both', radius=0.6, color=[1.0, 0.0, 1.0], axes=axes)

        cmd.set("cgo_line_radius", 0.03)  # 0.03
        cmd.load_cgo(cgo, 'txt')
        #ZOOM
        cmd.zoom("all", -10.0)  # Higher and positive values zoom  out, it accepts negative values
    ...
    pymol.cmd.png(os.path.dirname(PDB_file) + "/%s_Local" % (sname.split('.')[0]))
